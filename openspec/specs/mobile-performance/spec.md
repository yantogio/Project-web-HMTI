...
Defines cross-cutting requirements to keep the admin application light and smooth on mobile viewports by bounding GPU/compositing cost (gating expensive `backdrop-blur` and looping background animations), without changing desktop appearance or existing functionality.
