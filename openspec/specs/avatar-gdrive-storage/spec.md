# Spec: Avatar GDrive Storage

## Purpose

Defines requirements for uploading and storing member profile photos (avatars) in Google Drive, using the existing `GoogleDriveService`. Uploaded photos are stored in a dedicated Drive folder and the resulting public URL is persisted on the `Member` model for display across the application.
