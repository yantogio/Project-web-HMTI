...
const downloadDuesReport = async () => {
  if (!authStore.token) return
  isDownloading.value = true
  try {
    const res = await http.get('/finance/dues-report', {
      headers: { Authorization: `Bearer ${authStore.token}` },
      responseType: 'blob'
    })
    const url = URL.createObjectURL(new Blob([res.data]))
    const a = document.createElement('a')
    a.href = url
    a.download = 'Laporan-Uang-Kas.xlsx'
    a.click()
    URL.revokeObjectURL(url)
  } catch (error) {
    console.error(error)
    toastError('Gagal download laporan uang kas: ' + (error.response?.data?.message || error.message))
  } finally {
    isDownloading.value = false
  }
}
        ...
        <div :class="['p-6 rounded-2xl', themeClasses.cardGlass]">
          ...
          <div class="flex flex-wrap gap-4 items-end">
            ...
            <div class="flex gap-3 flex-wrap">
              <button v-if="isBendahara" @click="downloadDuesReport" :disabled="isDownloading"
                class="flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm bg-amber-600 hover:bg-amber-500 text-white shadow transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                {{ isDownloading ? 'Loading...' : 'Laporan Uang Kas' }}
              </button>
